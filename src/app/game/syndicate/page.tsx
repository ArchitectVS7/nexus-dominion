 ...
 * Syndicate Page
 *
 * Black Market interface for illicit trades and contracts.
 * Uses React Query for data fetching.
 *
 * @spec SPEC-SYNDICATE
 * @see docs/expansion/SYNDICATE.md
 */

import { useEffect } from "react";
import { useRouter } from "next/navigation";
import { useHasActiveGame, useGameLayout } from "@/lib/api";
...
/**
 * Locked state UI - displayed before Turn 50 when Syndicate is not yet available
 */
function SyndicateLockedView({ currentTurn }: { currentTurn: number }) {
  ...
  return (
    <div className="lcars-panel text-center py-12" data-testid="syndicate-locked">
      <div className="max-w-md mx-auto">
        ...
        <p className="text-gray-600 text-sm mb-4">— Unknown Contact</p>
...
export default function SyndicatePage() {
  ...
  const { data: layout, isLoading: layoutLoading } = useGameLayout();
  ...
  const isLoading = checkingGame || layoutLoading;
  const currentTurn = layout?.currentTurn ?? 1;
      ...
      <div className="max-w-6xl mx-auto px-4 md:px-0" data-testid="syndicate-page">
        <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start mb-6 md:mb-8 gap-2">
          ...
          <p className="text-xs md:text-sm text-gray-400 italic max-w-md">
            A shadowy network offering contracts, components, and forbidden
            weapons - for those willing to deal in the dark.
          </p>
        </div>
        <div
          className="lcars-panel animate-pulse"
          data-testid="syndicate-loading"
        >
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 md:px-0" data-testid="syndicate-page">
      ...
      {isBlackMarketUnlocked ? (
        ...
        <SyndicateLockedView currentTurn={currentTurn} />
      )}
    </div>
  );
}
