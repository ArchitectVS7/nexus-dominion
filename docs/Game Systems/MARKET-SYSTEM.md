...
### REQ-MKT-002: Dynamic Pricing

**Description:** Market prices fluctuate based on supply, demand, market events, and leader penalty using the formula:

```
Price = Base Price × Supply Modifier × Demand Modifier × Event Modifier × Leader Penalty
```

Prices update each turn with 5% decay toward baseline to prevent permanent inflation/deflation.

**Rationale:** Creates market dynamics and strategic timing decisions. Prevents static economy where prices never change.
...
|-----------|-------|-------|
| Market Depth | 100,000 units | Dampens extreme swings |
| Decay Rate | 5% per turn | Reverts to baseline |
| Price Floor | 50% of base | Minimum price |
| Price Ceiling | 200% of base | Maximum price |

**Source:** Section 2.2, 3.1

**Code:**
- `src/lib/market/pricing.ts` - `calculatePrice()`, `updatePriceModifiers()`

**Tests:**
- `src/lib/market/__tests__/pricing.test.ts` - Price calculation tests
...
### REQ-MKT-008: Bot Archetype Trading

**Description:** Each bot archetype has distinct trading behavior:
- **Merchant**: 0.95 trading priority, arbitrage focus
- **Warlord**: Buys Ore/Petroleum 2 turns before attacks
- **Turtle**: Maintains 20-turn food reserve
- **Schemer**: Hoards resources, manipulates prices

Bots respect transaction limits and fees like players.

**Rationale:** Creates realistic market activity. Bot trading generates supply/demand fluctuations and strategic opportunities.

**Source:** Section 4.1, 4.2

**Code:**
- `src/lib/bots/market-behavior.ts` - `botTradingDecision()`
- `src/lib/bots/archetypes/` - Per-archetype trading logic

**Tests:**
- `src/lib/bots/__tests__/market-behavior.test.ts` - Bot trading tests
