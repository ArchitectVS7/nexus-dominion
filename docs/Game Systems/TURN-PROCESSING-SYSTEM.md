...
### REQ-TURN-019: Victory Check Phase

**Description:** Victory Check Phase (Phase 16) evaluates all 6 victory conditions for all empires:
1. Military (70% sector control)
2. Economic (100,000 credits)
3. Diplomatic (alliance with 80% of empires)
4. Research (all tech tree branches complete)
5. Covert (5+ empires eliminated via assassination)
6. Survival (last empire standing)

First empire to meet any condition wins. Game locks (no more turns). Failures log errors but don't block turn.

**Rationale:** Multiple victory paths increase replayability. Evaluated at turn end for fairness.

**Key Values:**
| Victory Type | Threshold | Notes |
|--------------|-----------|-------|
| Military | 70% sectors | Roughly 70 of 100 sectors |
| Economic | 100,000 credits | Stockpile, not income |
| Diplomatic | 80% alliances | 80 of 100 empires allied |
| Research | 100% tech tree | All branches completed |
| Covert | 5 eliminations | Via assassination only |
| Survival | 1 empire left | Last one standing |

**Source:** Section 3.16

**Code:**
- `src/lib/game/services/phases/victory-check-phase.ts` - `processVictoryCheck()`
- `src/lib/game/victory/victory-conditions.ts` - Condition evaluation logic
- `src/lib/game/victory/victory-screen.ts` - Victory UI display

**Tests:**
- `src/lib/game/services/__tests__/victory-check-phase.test.ts` - Each victory condition, game lock
